...
class SongUNetConfig:
    img_resolution: int = 256   # Image resolution at input/output.
    ...
    channel_mult: list[int] = field(default_factory=lambda: [2, 2, 2])   
    channel_mult_emb: int = 4             # Multiplier for the dimensionality of the embedding vector.
    num_blocks: int = 2                   # Number of residual blocks per resolution.
...
class SongUNet(torch.nn.Module):
    ...
    def __init__(
        ...
        assert config.embedding_type in ["positional"]
        ...
        # Mapping.
        self.map_time = PositionalEmbedding(num_channels=time_channels, endpoint=False)
        ...
        self.map_x_cond = Conv2d(in_channels=config.in_channels, out_channels=config.model_channels, kernel=1, init_mode="xavier_uniform",init_weight=1, init_bias=0)
    ...
    @classmethod
    def from_pretrained(
        cls, save_directory: str, filename: str = "unet", use_ema: bool = False
    ):
        config_path = os.path.join(save_directory, f"{filename}_config.json")
    ...
    def forward(self, x, time, x_cond=None, class_labels=None, augment_labels=None):
        # Mapping.
        ...
        # Handle x_cond processing at the beginning
        x_cond_processed = False
        if x_cond is not None:
            if x_cond.shape[2:] != x.shape[2:]:
                x_cond = torch.nn.functional.interpolate(x_cond, size=x.shape[2:], mode='bilinear')
        ...
        for name, block in self.enc.items():
            if "aux_down" in name:
                ...
                if isinstance(block, UNetBlock):
                    ...
                    x = block(x)
                    # Apply x_cond conditioning after the first conv layer
                    if x_cond is not None and not x_cond_processed and "conv" in name:
                        x = x + self.map_x_cond(x_cond)
                        x_cond_processed = True
