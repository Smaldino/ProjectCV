...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Interpolation method for the rectified flow.",
    )
    parser.add_argument(
        "--source_distribution",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="The root directory where the city dataset is stored.",
    )
    parser.add_argument(
        "--data_target",
        type=str,
        default="./data",
        help="The directory where the city dataset target is stored.",
    )
    parser.add_argument(
        "--data_source",
        type=str,
        default=None,
        help="The root directory where the city dataset source is stored.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main(args):
    ...
    print(f"[DEBUG] Model moved to device {accelerator.device} with dtype {weight_dtype}")

    if args.use_ema:
        model_ema = EMAModel(model)
        print("[DEBUG] EMA model initialized.")

    # 2. Prepare datasets
    logger.info("******  Preparing datasets  ******")
    print(f"[DEBUG] Preparing datasets with source: {args.data_source}, target: {args.data_target}")
    ...
    print(f"[DEBUG] Source dataset size: {len(dataset_source)}, Target dataset size: {len(dataset_target)}")
    ...
    print(f"[DEBUG] CouplingDataset size: {len(train_dataset)}")
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=partial(coupling_collate_fn, D0_distribution=None),  # D0 is provided by dataset
        pin_memory=True,
    )
    print(f"[DEBUG] DataLoader created with batch size {args.train_batch_size}")
